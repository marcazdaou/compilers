...
class Parser {
    ...
    /**
     * Parses for statement and returns a list of ASTs.
     * 
     * <pre>
     ...
     */
     ...
     private ArrayList<JStatement> forInit() {
        ...
        if (!seeLocalVariableDeclaration()) {
            // Add statement expressions if it's not a local variable declaration
            statements.add(statementExpression());
        while (have(COMMA)) {
            statements.add(statementExpression());
        }
        } else {
            // If it's a local variable declaration, add JVariableDeclaration
            int line = scanner.token().line();
    ...
    /**
     * Parses the update of the for loop and returns an AST.
     * 
     * <pre>
    ...
    private ArrayList<JStatement> forUpdate() {
        ArrayList<JStatement> statements = new ArrayList<JStatement>(); // arraylist
    
        // Add statement expressions for the update part (similar to forInit)
        statements.add(statementExpression());  // add expre
        while (have(COMMA)) {  // while loop
            statements.add(statementExpression());  
        }
    ...
    private JStatement statement() {
        ...
        if (see(LCURLY)) {
        ...
        } else if (have(BREAK)) {  // Handling the break statement
                mustBe(SEMI);  // semicolon
                return new JBreakStatement(line);  // Return JBreakStatement
        }else if (have(CONTINUE)) {  // continue statement
                    mustBe(SEMI);  // semi
                    return new JContinueStatement(line);  // reutrn
        } else if (have(SEMI)) {
            return new JEmptyStatement(line);


        } else if (have(DO)) {
            JStatement something = statement();  // body 
            mustBe(WHILE);  // must have WHILE
            JExpression another = parExpression(); 
            mustBe(SEMI);  // must have semi
            return new JDoStatement(line, something, another);
            ...
            // Parse initialization if it's not just a semicolon
            ArrayList<JStatement> inits = null;
            if (!see(SEMI)) {
                inits = forInit();
            }
            mustBe(SEMI); // semicolon after initialization
        
            // Parse condition if it's not just a semicolon
            JExpression condition = null;
            if (!see(SEMI)) {
                condition = expression();
            }
            mustBe(SEMI); // semicolon after condition
        
            // Parse updates if it's not the closing parenthesis
            ...
            JStatement body = statement(); // Body of the for loop
        
            // Return the JForStatement object with parsed parts
            return new JForStatement(line, inits, condition, updates, body);
        } else if (have(SWITCH)) {
            mustBe(LPAREN); //  '(' after SWITCH
            JExpression expr = expression(); // Parse the switch expression
            mustBe(RPAREN); //  ')' after expression
            mustBe(LCURLY); //  '{' to start the switch block
    
            ArrayList<SwitchStatementGroup> groups = new ArrayList<>(); // Store switch groups
    
            // Parse each case/default block in the switch
            while (!see(RCURLY) && !see(EOF)) {
                groups.add(switchBlockStatementGroup()); // Parse each switch block group
            }
    
            mustBe(RCURLY); //  '}' at the end of the switch block
            return new JSwitchStatement(line, expr, groups); // Return the parsed switch statement
        } else {
            // Otherwise, it's an expression statement
            JStatement statement = statementExpression();
            mustBe(SEMI); // Expect semicolon
            return statement;
        }
    }
    ...
    private JExpression variableInitializer(Type type) {
     ...
     /** 
     * Parses a switch statement group and returns an AST.
     * 
     * <pre>
    ...
    */
    private SwitchStatementGroup switchBlockStatementGroup() {
        ArrayList<JExpression> labels = new ArrayList<>(); // Store case/default labels
        labels.add(switchLabel()); // Parse the first label
    
        while (see(CASE) || see(DEFAULT)) {
            labels.add(switchLabel()); // Parse more labels if any
        }
    
        ArrayList<JStatement> statements = new ArrayList<>(); // Store statements under the labels
        while (!see(CASE) && !see(DEFAULT) && !see(RCURLY)) {
            statements.add(statement()); // Parse the statements until we hit a new case/default or closing brace
        }
    
        return new SwitchStatementGroup(labels, statements); // Return the group with labels and statements
    }
    

     /**
     * Parses a switch label and returns an AST .
     *
     * <pre>
     ...
     */
     ...
     private JExpression switchLabel() {
        if (have(CASE)) {
            JExpression label = expression(); // Parse the case expression
            mustBe(COLON); //  colon after the case expression
            return label;
        } else if (have(DEFAULT)) {
            mustBe(COLON); // colon after default
            return null; // No expression for default
        } 
        return null;
    }
    ...
    private Type basicType() {
        if (have(BOOLEAN)) {
            ...
            return Type.INT;
        } else if (have(LONG)) {  // Added support for 'long'
        return Type.LONG;
    } else if (have(DOUBLE)) {  // Added support for 'double'
        return Type.DOUBLE;
        } else {
    ...
    private JExpression assignmentExpression() {
        int line = scanner.token().line();
        JExpression lhs = conditionalExpression();  // remove the AND from the Expression
        if (have(ASSIGN)) {
        ...
        return lhs;  // Return LHS if no assignment
    }
    ...
    private JExpression multiplicativeExpression() {
        ...
        while (more) {
            ...
            } else if (have(REM)) {   // for the remainder opp
                lhs = new JRemainderOp(line, lhs, unaryExpression());
                more = false;
            } else if (have(DIV)) {  // for the division opp
    ...
    private JExpression unaryExpression() {
        ...
        } else if (have(PLUS)) {  // for the plus operation
    ...
    private JExpression literal() {
        ...
        } else if (have(LONG_LITERAL)) {  // Handle long literals 
            return new JLiteralLong(line, scanner.previousToken().image());
        } else if (have(DOUBLE_LITERAL)) {  // Handle double literals 
    ...
    private boolean seeBasicType() {
        return (see(BOOLEAN) || see(CHAR) || see(INT) || see(LONG) || see(DOUBLE)); // added long and double
    }
    ...
    private boolean seeReferenceType() {
        if (see(IDENTIFIER)) {
            ...
            scanner.recordPosition();
            if (have(BOOLEAN) || have(CHAR) || have(INT) || have(LONG) || have(DOUBLE)) { // added long and double
